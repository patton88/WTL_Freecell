...
void CWTLfcData::AutoThrow()
{
	...
	while (true) { //直到没有牌可扔为止
		...
		{ //扔牌后的自动扔牌动作必须和扔牌动作放在一起
			...
			m_OpsList.m_tList.back().AddOperation(colDes, colSrc, 1);
		}
	}
}
...
void CWTLfcData::Record(UINT des, UINT src, UINT n)
{
	...
	m_OpsList.m_tList.push_back(COperations(des, src, n));
...
void CWTLfcData::Undo()
{
	...
	if (m_OpsList.m_tList.empty()) return;

	//撤销一步
	//COperations *pOpsLast = (COperations*)m_pOps->back();
	COperations& OpsLast = m_OpsList.m_tList.back();
	// GetTail	返回列表中的尾元素（列表不能为空）
...
{
	int nSteps = m_OpsList.m_tList.size();

	//还原牌局但保留步骤记录
	while (nSteps > 0)
	{
		COperations* pOpsLast = getAt(m_OpsList.m_tList, --nSteps);
...
bool CWTLfcData::AICal()
{
	...
	//此局很可能导致算法无限死循环
	if (!m_OpsList.m_tList.empty() && m_OpsList.m_tList.size() > 200) {
		//g_bStopAICal = true;
		return false;
	}
	AutoThrow();
...
BOOL CWTLfcData::GiveUp()
{
	...
	if (m_OpsList.m_tList.empty() || GameOver()) return true;
	//否则要提醒玩家是否放弃当前局
