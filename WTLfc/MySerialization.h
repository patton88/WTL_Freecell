...
public:
	...
	static inline istream& read_internal(istream& istream_, char* p, unsigned size)
	{
		if (!LE())
		{
	...
	static inline ostream& write_internal(ostream& ostream_, const char* p, unsigned size)
	{
		if (!LE())
		{
	...
	static inline istream& read(istream& istream_, CPoint& pt_)
	{
		//int size = 0;
		//read(istream_, size);
	...
	static inline istream& read(istream& istream_, vector<bool>&container)
	{
		...
		// 增加该句，解决 ifstream 流已到文件末尾，还需读一次eof()才为真的问题
		if (!istream_.good() || istream_.eof())return istream_;
	...
	static inline ostream& write(ostream& ostream_, CPoint& pt_)
	{
		//unsigned size = sizeof(pt_);
		//write(ostream_, size);

		write(ostream_, pt_.x);
		write(ostream_, pt_.y);
		return ostream_;
	}
	...
	static inline istream& read(istream& istream_, T& t_)
	{
		...
		if (std::is_fundamental<T>::value || typeid(CPoint) == typeid(T))
		{
		...
		else
		{
			if (std::is_trivial<T>::value)
				...
				return read(istream_, (MySerialize::I*)&t_);
		}
		assert(0);
		return istream_;
	}
	...
	static inline ostream& write(ostream& ostream_, T& t_)
	{
		...
		else
		{
			if (std::is_trivial<T>::value)
				...
				return write(ostream_, (MySerialize::I*)&t_);
		}
	}
	...
	static inline ostream& write(ostream& ostream_, vector<T*>& container)
	{
		...
		write_internal(ostream_, (char*)&size, sizeof(unsigned));
		int index = 0;
